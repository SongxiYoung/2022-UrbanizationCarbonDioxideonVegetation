...
%     V(54).Val   = input_var(2);      % SMC->Volumetric soil moisture content
    % leaf optical
    V(6).Val   = input_var(1);      % N->Mesophyll structural parameter in prospect
    V(4).Val  = input_var(2);       % Cw->Water content in PROSPECT
    V(3).Val    = input_var(3);       % Cdm->Dry matter content in PROSPECT
    V(5).Val    = input_var(4);      % Cs->Senescence factor in PROSPECT
    V(1).Val    = input_var(5);      % Cab->Chlorophyll content in PROSPECT
    % leaf physiology(TB12-D, TB12, MD12)
    V(10).Val     = input_var(6);      % m->Ball?Berry stomatal conductance parameter
    V(12).Val   = input_var(7);      % kV->Extinction coefficient for a vertical profile of Vcmo (maximum value of Vcmo occurs at the top of the canopy).
    V(13).Val   = input_var(8);     % Rdparam->Parameter for dark respiration (Rd = Rdparam ? Vcmo)
    V(9).Val  = input_var(9);      % Vcmo->maximum carboxylation capacity (at optimum temperature)
    % Leaf physiology (MD12 only)
    V(57).Val= input_var(10);      % kNPQs->Rate constant of sustained thermal dissipation
    V(58).Val    = input_var(11);     % qLs->Fraction of functional reaction centers
%     % Aerodynamic
...
%     % Micrometeorologic
%     V(33).Val   = input_var(1);     % p->Air pressure
%     V(35).Val  = input_var(2);      % u->Wind speed
%     V(37).Val= input_var(3);      % Oa->O2 concentration in the air
%     V(34).Val    = input_var(4);     % ea->Atmospheric vapor pressure
%     V(36).Val    = input_var(5);     % Ca->CO2 concentration in the air
%     V(31).Val     = input_var(6);     % Ta->Air temperature
%     V(30).Val   = input_var(7);     % Rin->Incoming shortwave radiation
%     V(32).Val   = input_var(7);     % Rli->Incoming longwave radiation
%     % Canopy
%     V(28).Val   = input_var(7);     % lw->Leaf width
%     V(26).Val  = input_var(8);      % LIDFa->LIDF parameter a, which controls the average leaf slope 
%     V(27).Val= input_var(9);      % LIDFb->LIDF parameter b, which controls the distribution's bimodality
%     V(22).Val    = input_var(10);     % LAI->Leaf area index
%     V(23).Val    = input_var(11);     % hc->Canopy height
%     % Geometry
%     V(52).Val     = input_var(5);     % VZA->Viewing zenith angle
%     V(51).Val   = input_var(6);     % SZA->Sun zenith angle
%     V(53).Val   = input_var(7);     % RAA->Relative azimuth angle
